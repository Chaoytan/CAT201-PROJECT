<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Restaurant Admin Dashboard</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <link rel="stylesheet" href="css/index.css">
</head>
<body>
]
<nav class="navbar">
    <div class="brand-name">
        <i class="fa-solid fa-layer-group"></i> KOPITIAM ADMIN
    </div>
    <div class="user-profile">
        <i class="fa-solid fa-circle-user"></i> Hello, Albert
    </div>
</nav>

<div class="main-content">

    <div class="welcome-section">
        <h1>Overview</h1>
        <p>Welcome back. Here is what's happening at your restaurant today.</p>
    </div>

    <div class="action-container">
        <a href="Admin-Menu.jsp" class="action-card">
            <i class="fa-solid fa-utensils"></i>
            <span>Menu Management</span>
        </a>

        <a href="view-feedback.jsp" class="action-card">
            <i class="fa-solid fa-comments"></i>
            <span>Feedback Inbox</span>
        </a>

        <a href="Link_To_CKC.jsp" class="action-card">
            <i class="fa-solid fa-comments"></i>
            <span>CKC Page</span>
        </a>

    </div>

    <div class="stats-container">

        <a href="ViewOrders.jsp" style="text-decoration: none; color: inherit;">
            <div class="stat-card blue-accent">
                <div class="icon-box"><i class="fa-solid fa-clipboard-check"></i></div>
                <div class="text-box">
                    <%
                        int orderCount = 0;
                        try {
                            // 1. Load the Oracle Driver
                            Class.forName("oracle.jdbc.driver.OracleDriver");

                            // 2. CONNECT WITH YOUR CORRECT DETAILS
                            String dbUrl = "jdbc:oracle:thin:@localhost:1521:xe";
                            String dbUser = "kopitiam";      // Correct Username
                            String dbPass = "password123";   // Correct Password

                            java.sql.Connection con = java.sql.DriverManager.getConnection(dbUrl, dbUser, dbPass);

                            // 3. Count today's orders - UPDATED FOR YOUR TABLE STRUCTURE
                            String sql = "SELECT COUNT(*) FROM ORDERS WHERE TRUNC(ORDER_DATE) = TRUNC(SYSDATE)";

                            java.sql.PreparedStatement stmt = con.prepareStatement(sql);
                            java.sql.ResultSet rs = stmt.executeQuery();

                            if(rs.next()){
                                orderCount = rs.getInt(1);
                            }
                            con.close();
                        } catch(Exception e){
                            e.printStackTrace(); // Prints error to IntelliJ console if it fails
                        }
                    %>
                    <h3><%= orderCount %></h3>
                    <p>Orders Today</p>
                </div>
            </div>
        </a>

        <a href="RevenueDetails.jsp" style="text-decoration: none; color: inherit;">
            <div class="stat-card green-accent">
                <div class="icon-box"><i class="fa-solid fa-dollar-sign"></i></div>
                <div class="text-box">
                    <%
                        double totalRevenue = 0;
                        try {
                            Class.forName("oracle.jdbc.driver.OracleDriver");
                            String dbUrl = "jdbc:oracle:thin:@localhost:1521:xe";
                            String dbUser = "kopitiam";
                            String dbPass = "password123";

                            java.sql.Connection con = java.sql.DriverManager.getConnection(dbUrl, dbUser, dbPass);

                            String sql = "SELECT NVL(SUM(TOTAL_AMOUNT), 0) FROM ORDERS WHERE TRUNC(ORDER_DATE) = TRUNC(SYSDATE)";

                            java.sql.PreparedStatement stmt = con.prepareStatement(sql);
                            java.sql.ResultSet rs = stmt.executeQuery();

                            if(rs.next()){
                                totalRevenue = rs.getDouble(1);
                            }
                            con.close();
                        } catch(Exception e){
                            e.printStackTrace();
                        }
                    %>
                    <h3>$<%= String.format("%.2f", totalRevenue) %></h3>
                    <p>Revenue Today</p>
                </div>
            </div>
        </a>

    </div>
</div>

<div class="footer">
    © 2026 Kopitiam Admin System
</div>

</body>
</html>